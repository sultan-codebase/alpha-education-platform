...
export interface Student {
  id: number
  name: string                // ← склеенное ФИО
  iin: string
  course?: string             // курс (например, "Data Science")
  stream: string              // поток / финансирование (например, "techorda", "A1")
  topStudent: boolean
  email?: string
  phone?: string
  status?: string             // например, "студент" или "выпускник"
  financing?: string          // тип финансирования
  comment?: string
}
...
export const useStudentStore = defineStore('student', {
  ...
  actions: {
    ...
    /** Экспортировать список студентов в Excel */
    exportToExcel() {
    ...
    async fetchById(id: string): Promise<StudentFull> {
      ...
      // преобразуем в формат PaymentEntry
      ...
      // определим скидку по stream или financing
